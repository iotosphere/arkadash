...
{
    /* Sadece AI konuşurken animasyon göster - kayıt sırasında gizle */
    if (!is_ai_speaking) {
        if (lvgl_port_lock(0)) {
    ...
    if (is_recording) {
        switch (p) {
            case 0: l=80; break;
            case 1: l=40; c=80; break;
            case 2: c=100; break;
            case 3: c=80; r=40; break;
            case 4: r=80; break;
            case 5: c=40; r=40; break;
        }
    } else {
...
static void chat_task(void *pv)
{
    ...
    while (1) {
        ...
        if ((cur == objects.chat || cur == objects.agent) && !display_sleeping) {
            ...
            if (k0 && !last_k0 && rx && tx) {
                ...
                while (key0_pressed() && is_recording && s_rec_bytes < REC_BUF_SIZE) {
                    size_t ck = (s_rec_bytes + 2048 > REC_BUF_SIZE)
                                    ? (REC_BUF_SIZE - s_rec_bytes) : 2048;
                    size_t rd = 0;
                    esp_err_t ret = i2s_channel_read((i2s_chan_handle_t)rx,
                                                     s_rec_buf + s_rec_bytes,
                                                     ck, &rd,
                                                     pdMS_TO_TICKS(1000));
                    if (ret == ESP_OK && rd > 0) {
                        s_rec_bytes += rd;
                ...
                ESP_LOGI(TAG, "REC STOP %lu ms %d bytes",
                         (unsigned long)held, (int)s_rec_bytes);
                ...
                if (held > 100 && s_rec_bytes > 4000) {
                    ...
                    if (ws_send_audio(s_rec_buf, s_rec_bytes) == ESP_OK) {
                        ...
                        int     empty_count  = 0;
                        int     timeout_count = 0;
                        const int MAX_EMPTY   = 200;   /* 1s boş sonra tts_end  */
                        const int MAX_TIMEOUT = 6000;  /* 30s toplam zaman aşımı */

                        while (timeout_count < MAX_TIMEOUT) {
                            size_t chunk = ws_stream_read(play_buf, sizeof(play_buf), 20);
                            if (chunk > 0) {
                                size_t    written = 0;
                                esp_err_t wret = i2s_channel_write(
                                    (i2s_chan_handle_t)tx,
                                    play_buf, chunk, &written,
                                    pdMS_TO_TICKS(100));
                            ...
                            if (ws_is_tts_complete() && empty_count > MAX_EMPTY) {
                                ESP_LOGI(TAG, "Streaming complete, played %d bytes",
                                         (int)total_played);
                                break;
                            }

                            timeout_count++;
                            kitt_anim();  // AI konuşurken KITT animasyonu güncelle
                        ...
                        ESP_LOGI(TAG, "PLAY DONE: %d bytes (%.2fs)",
                                 (int)total_played, actual_dur);
...
static void encoder_read(lv_indev_t *indev, lv_indev_data_t *data)
{
    ...
    /* Ekran uyku modundayken herhangi bir basışla uyandır */
    ...
    if (!display_sleeping) {
        ...
        if (cur == objects.games) data->enc_diff = 0;
        else                      data->enc_diff = diff;
    } else {
        data->enc_diff = 0;
    }
    ...
    if (cur == objects.menu) {
        if (pushed) {
            ...
            if (menu_just_loaded) { menu_just_loaded = false; push_handled = false; }
    ...
    if (cur == objects.games) {
        ...
        if (pushed) {
            if (!push_handled) { push_handled = true; push_start_ms = lv_tick_get(); }
            if (lv_tick_get() - push_start_ms >= LONG_PRESS_MS) {
                if (lvgl_port_lock(0)) {
                    ...
                    if (objects.chat_button) lv_group_focus_obj(objects.chat_button);
                    encoder_reset_count();
            ...
            if (push_handled && !long_press_triggered &&
                ...
                if (lvgl_port_lock(0)) { bricks_breaker_start(); lvgl_port_unlock(); }
            }
            push_handled         = false;
            long_press_triggered = false;
        }
...
void app_main(void)
{
    ...
    const lvgl_port_display_cfg_t dcfg = {
        ...
        .buffer_size   = 240 * 32,  // Increased for better performance
        .double_buffer = false,
        .hres          = 240,
        .vres         = 320,
    ...
    xTaskCreatePinnedToCore(display_task, "display_task", 16384, NULL, 5, NULL, 0);
